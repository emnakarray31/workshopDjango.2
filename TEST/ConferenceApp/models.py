...
class SUBMISSION(models.Model):
    ...
    def clean(self):
        ...
        # Vérifier que la soumission concerne une conférence à venir
        if self.conference and self.conference.start_date <= today:
            raise ValidationError(
                f"La conférence « {self.conference.name} » commence le {self.conference.start_date}, "
                f"il est donc trop tard pour soumettre."
            )
        ...
        if same_day_submissions >= 3:
            raise ValidationError(
                "Vous ne pouvez pas soumettre à plus de 3 conférences le même jour."
            )
